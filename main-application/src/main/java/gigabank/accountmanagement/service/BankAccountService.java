...
public class BankAccountService {
     ...
     * @return страница с банковскими счетами в формате DTO
     */
    @Cacheable(value = "accountsCache", key = "'user_' + #userId")
    public List<BankAccountResponse> getUserAccounts(Long userId) {
        log.info("Попытка найти все счета пользователя: {}.", userId);
        List<BankAccountEntity> bankAccountEntities = bankAccountRepository.findAllByOwnerId(userId);
        log.info("Найдено {} счетов пользователя {}.", bankAccountEntities.size(), userId);

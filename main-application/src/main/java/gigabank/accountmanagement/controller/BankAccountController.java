...
public class BankAccountController {
    ...
    /**
     * Получает список всех счетов по указанному пользователю.
     *
     * @param userId уникальный идентификатор пользователя
     * @return ResponseEntity со списком DTO счетов
     */
    @GetMapping("/user/{userId}")
    public ResponseEntity<List<BankAccountResponse>> getUserAccounts(@PathVariable Long userId) {
        return ResponseEntity.ok(bankAccountService.getUserAccounts(userId));
    }

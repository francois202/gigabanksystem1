...
public class BankTransferStrategy implements PaymentStrategy {
    ...
    public BankTransferStrategy(PaymentGatewayService paymentGatewayService, ExternalNotificationService externalNotificationService) {
    ...
    public void process(BankAccount account, BigDecimal amount, Map<String, String> details) {
        ...
        paymentGatewayService.authorize("Платеж через банк", amount);
        externalNotificationService.sendSms(adapter.getPhone(), "Произошел платеж через банк");
        externalNotificationService.sendEmail(adapter.getEmail(), "Информация о платеже", "Произошел платеж через банк");
    }
}
